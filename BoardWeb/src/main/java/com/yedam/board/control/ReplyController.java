...
public class ReplyController {
	...
	// value = url, produces = 생산되어지는 미디어 타입
	public ResponseEntity create(@RequestBody ReplyVO vo) {
		// String : 성공 or 실패 문자열 // ResponseRntity : 성공 + code, 실패 + code(잘못된 요청입니다)
		int result = replyService.register(vo);
		if (result == 1) {
			return new ResponseEntity<>("성공", HttpStatus.OK); // HttpStatus.OK = 성공값 (200)
			// return "성공";
		} else {
